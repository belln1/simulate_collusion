...
get_detection_ind <- function(periods, sigma, seed){
...
get_detection_model2_ind <- function(periods, sigma, d_nfirms, seed){
  #set.seed(seed)
...
simulate_industry <- function(i, parms, k, seed_start, model, min_share) {
  ...
  count <- 1
  all_ind_delta <- get_deltas_r(allperiods, seed=count)
  ...
  if (model==2){
    detection <- get_detection_model2_ind(allperiods, sigma, parms$d_nfirms, seed=count)
  }else {
    detection <- get_detection_ind(allperiods, sigma, seed=count)
  }
  ...
  return(list(cartels = in_cartel, detection = detection, ICC_entry = ICC_entry, ICC_exit = ICC_exit, sigma_cartels = sigma, deltas = all_ind_delta))
}

# INDUSTRYLEVEL - MODEL III
simulate_industry_model3 <- function(i, parms, k, seed_start, min_share) {
  gamma = parms$gamma
    ...
    
  {
    ...
    if(sum(in_cartel[j] * detection[j])>0){
      ...
      if (parms$structured & (allperiods-j)>1) {
        ...
        detection[range] <- get_detection_ind(allperiods-j, sigma[range], seed=count)
      }
      # No cartel in next period (if there are next periods)
      if(j<allperiods){
        in_cartel[j+1] = 0
