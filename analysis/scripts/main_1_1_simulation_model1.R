...
#    sim_list <- simulate_firms(i, parms[k,], k, sim_seed, model=1, min_share) 
    sim_list <- simulate_industry(i, parms[k,], k, sim_seed, model=1, min_share) 
...
y <- as.vector(delta_all)
z <- data.frame(y)
out <- get_sumstats(z)
save_kable(out, file = paste("analysis/data/", directory, "/sumstats_deltas.tex", sep = ""))
